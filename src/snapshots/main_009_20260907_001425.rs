...
// Goal: Using File IO: Read a directory content
...
fn main() {
    ...
    let read_dir_result: Result<fs::ReadDir, io::Error> = fs::read_dir(PATH_NAME);

    if let Err(err) = read_dir_result {
        panic!("An error Occur: {}", err);
    }

    let dir_read: fs::ReadDir = read_dir_result.unwrap();

    for item in dir_read {
        let de: fs::DirEntry = item.unwrap();
        if de.path().is_dir() {
            println!(" -->> {}", de.file_name().into_string().unwrap());
        }
